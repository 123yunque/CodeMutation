...
output_path = os.path.join(current_dir, "sample_output_LLMs_non_equivalent_code", "minimax")
...
def get_client(api_key):
    if not hasattr(thread_local, "client"):
        thread_local.client = OpenAI(api_key=api_key, base_url=NVIDA_URL)
...
def process_folder(folder, api_key):
    ...
    #         if num > 226:
    #             return f"⏭ 跳过 {folder}"
    #         if num not in [6, 9, 111, 118, 140, 16, 162, 167, 223, 87, 100, 101, 106, 108, 61, 65, 71, 88]:
    #             return f"⏭ 跳过 {folder}"
    #     except:
    #         pass
    ...
    combined_file = os.path.join(sub_dir, "sample_inputs_non_equivalent.py")
    if not os.path.exists(combined_file):
        return f"⚠️ 无 sample_inputs_non_equivalent.py: {folder}"
...
def main():
    with open(config_path, "r", encoding="utf-8") as f:
        api_key = json.load(f).get("NVIDA_3")
