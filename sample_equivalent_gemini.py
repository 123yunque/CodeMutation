...
output_path = os.path.join(current_dir, "sample_output_LLMs_equivalent_code", "minimax")
...
def process_folder(folder, api_key):
    ...
    combined_file = os.path.join(sub_dir, "sample_inputs_equivalent.py")
    if not os.path.exists(combined_file):
        return f"⚠️ 无 sample_inputs_equivalent.py: {folder}"
...
def main():
    with open(config_path, "r", encoding="utf-8") as f:
        api_key = json.load(f).get("NVIDA_2")
